...
Формат выходных данных Для каждого запроса выведите в отдельной строке слово "Yes", если класс 1 является предком
...
def is_parent(a, b):
    ...
    if a in relations[b]:
        ...
        return any(is_parent(a, sub) for sub in relations[b])


for i in range(int(input())):
    r = input().split(" : ")
    relations[r[0]] = r[1].split(' ') if len(r) == 2 else []

for i in range(int(input())):
    parent, target = input().split(' ', 1)
    if is_parent(parent, target):
        print('Yes')
    else:
        print('No')
